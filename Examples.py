...
for i in range(1,len(t)):
    dt = t[i] - t[i-1]
    print(i)
    n[i,:] = para.distribs.fp_findif_difu(dt, g, n[i-1,:], gdot[i-1,:], D, Qinj, t_esc, tlc) #Full Eq.
    #n[i, :] = para.distribs.fp_findif_difu(dt, g, n[i - 1, :], tlc) #1. no diff, esc, or inj.
    # j_s[i,:],ambs[i,:] = para.radiation.syn_emissivity_full(f,g,n[i,:],B,with_abs) #,sync and absorb
    # I_s[i,:] = para.radiation.radtrans_blob(j_s[i,:],R,ambs[i,:])
    # j_ssc[i,:] = para.radiation.ic_iso_powlaw_full(f,I_s[i,:],g,n[i,:])
    # I_ssc[i, :] = para.radiation.radtrans_blob(j_ssc[i, :], R, ambs[i, :])
...
for i in range(1,len(t)):
    ...
    print(i)
    n[i,:] = para.distribs.fp_findif_difu(dt, g, n[i-1,:], gdot[i-1,:], D, Qinj, t_esc, tlc) #Full Eq.
    #n[i, :] = para.distribs.fp_findif_difu(dt, g, n[i - 1, :], gdot[i - 1, :], tlc) #2. no diff, esc, or inj, cooling included.
    j_s[i,:],ambs[i,:] = para.radiation.syn_emissivity_full(f,g,n[i,:],B,with_abs) #,sync and absorb
    I_s[i,:] = para.radiation.radtrans_blob(j_s[i,:],R,ambs[i,:])
    j_ssc[i,:] = para.radiation.ic_iso_powlaw_full(f,I_s[i,:],g,n[i,:])
    I_ssc[i, :] = para.radiation.radtrans_blob(j_ssc[i, :], R, ambs[i, :])
...
'''
#3. No D, Qinj or gdot, but t_esc included
##define fp terms
#Qinj = broken_pwl(n0,g,p1,p2,gcut,g2cut)/t_inj
Qinj = np.full(numg,0)
D = np.full(numg, 0)
...
for i in range(1,len(t)):
    ...
    print(i)
    n[i,:] = para.distribs.fp_findif_difu(dt, g, n[i-1,:], gdot[i-1,:], D, Qinj, t_esc, tlc) #Full Eq.
    #n[i, :] = para.distribs.fp_findif_difu(dt, g, n[i - 1, :], t_esc, tlc) #3. no diff, esc, inj, or cooling, but t_esc included.
    j_s[i,:],ambs[i,:] = para.radiation.syn_emissivity_full(f,g,n[i,:],B,with_abs) #,sync and absorb
    I_s[i,:] = para.radiation.radtrans_blob(j_s[i,:],R,ambs[i,:])
    j_ssc[i,:] = para.radiation.ic_iso_powlaw_full(f,I_s[i,:],g,n[i,:])
    I_ssc[i, :] = para.radiation.radtrans_blob(j_ssc[i, :], R, ambs[i, :])
...
'''
#4. No D, gdot, or t_esc, but Qinj inlcuded
##define fp terms
Qinj = broken_pwl(n0,g,p1,p2,gcut,g2cut)/t_inj
Qinj = np.full(numg,0)
D = np.full(numg, 0)
